...
| [`@hugeicons/react`](packages/@hugeicons/react/) | `1.1.6` | Bun, npm (patch-package), pnpm | Ship `dist/types/core-free-icons.d.ts` with `declare module "@hugeicons/core-free-icons/*"` typed as `IconSvgElement` plus a `/// <reference path="./core-free-icons.d.ts" />` directive prepended to `dist/types/index.d.ts` so subpath type imports resolve under `node16`/`nodenext`/`bundler` resolution; fills the gap where `@hugeicons/core-free-icons` ships 5,100+ per-icon JS files but only a barrel `index.d.ts`, cutting dev pre-bundle from 6.2 MB to 33 KB for subpath-imported projects | [hugeicons/react#5](https://github.com/hugeicons/react/pull/5) |
