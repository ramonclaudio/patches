...
| [`@convex-dev/better-auth`](packages/@convex-dev/better-auth/) | `0.11.4` | Bun, npm (patch-package), pnpm | Invalidate `cachedToken` and `pendingTokenRef` on session rotation, not just logout; when `sessionId` flips with `session` still truthy, the stale cached JWT wins the `fetchAccessToken` short-circuit until the next forced refresh; companion to [better-auth/better-auth#9087](https://github.com/better-auth/better-auth/pull/9087) | [get-convex/better-auth#329](https://github.com/get-convex/better-auth/pull/329) |
| [`better-auth`](packages/better-auth/) | `1.6.2` | Bun, npm (patch-package), pnpm | Add `/change-password` and `/revoke-other-sessions` to the default `atomListeners` matcher in the client so `$sessionSignal` fires after session-rotating endpoints; without this, `useSession()` returns the stale session until reload; companion to [get-convex/better-auth#329](https://github.com/get-convex/better-auth/pull/329) | [better-auth/better-auth#9087](https://github.com/better-auth/better-auth/pull/9087) |
...
| [`@convex-dev/better-auth`](packages/@convex-dev/better-auth/) | `0.11.4` | Bun, npm (patch-package), pnpm | Migrate to `better-auth` 1.6.2: bumps peer to `>=1.6.2 <1.7.0`; accepts the new `Where.mode` field in adapter validators and case-folds `eq`/`ne`/`in`/`not_in`/`contains`/`starts_with`/`ends_with` for insensitive mode (excludes insensitive clauses from the indexable set since Convex indexes are byte-compared) so `api.adapter.findOne` stops throwing `ArgumentValidationError`; passes `asResponse: false, returnHeaders: false, returnStatus: false` at all 7 internal plugin endpoint call sites (convex + cross-domain) so 1.6's flipped `shouldReturnResponse` default doesn't turn `{ token }` into `undefined` (JWT cookies becoming the literal string `"undefined"`) or crash `setSessionCookie` on cross-domain; adds `twoFactor.verified` field for 1.6.2 compat; silences `oidcProvider` deprecation warning with `__skipDeprecationWarning`; exposes `version` on all 4 plugins | [get-convex/better-auth#323](https://github.com/get-convex/better-auth/pull/323) |
...
| [`@astrojs/compiler`](packages/withastro/compiler-rs/) | n/a | Source (git apply) | Add `-x` to `x86_64-unknown-linux-gnu` build for glibc compat | merged ([withastro/compiler-rs#22](https://github.com/withastro/compiler-rs/pull/22)) |
...
| [`app-store-connect-cli`](packages/rudrankriyam/) | n/a | Source (git apply) | macOS app screen capture + Mac App Store canvas framing for `shots` command | merged ([rudrankriyam/App-Store-Connect-CLI#784](https://github.com/rudrankriyam/App-Store-Connect-CLI/pull/784)) |
