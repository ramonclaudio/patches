...
| [`@convex-dev/better-auth`](packages/@convex-dev/better-auth/) | `0.11.4` | Bun, npm (patch-package), pnpm | Migrate to `better-auth` 1.6.5: bumps peer to `>=1.6.5 <1.7.0`; accepts the new `Where.mode` field in adapter validators and case-folds `eq`/`ne`/`in`/`not_in`/`contains`/`starts_with`/`ends_with` for insensitive mode (excludes insensitive clauses from the indexable set since Convex indexes are byte-compared) so `api.adapter.findOne` stops throwing `ArgumentValidationError`; passes `asResponse: false, returnHeaders: false, returnStatus: false` at all 7 internal plugin endpoint call sites (convex + cross-domain) so 1.6's flipped `shouldReturnResponse` default doesn't turn `{ token }` into `undefined` (JWT cookies becoming the literal string `"undefined"`) or crash `setSessionCookie` on cross-domain; delegates cross-domain `parseSetCookieHeader` to `better-auth/cookies` so Set-Cookie headers with RFC-1123 `Expires` dates stop shattering into garbage entries (local copy pre-dated better-auth's 1.6.0 lookahead fix, [#8301](https://github.com/better-auth/better-auth/pull/8301)); adds `twoFactor.verified` field for 1.6.2 compat; silences `oidcProvider` deprecation warning with `__skipDeprecationWarning`; exposes `version` on all 4 plugins; picks up `@better-auth/oauth-provider` [GHSA-xr8f-h2gw-9xh6](https://github.com/better-auth/better-auth/security/advisories/GHSA-xr8f-h2gw-9xh6) (high-severity authz bypass, patched in 1.6.5) via the peer bump | [get-convex/better-auth#323](https://github.com/get-convex/better-auth/pull/323) |
