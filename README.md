...
| :--- | :--- | :--- | :--- |
| [`@expo/ui`](packages/@expo/ui/) | `56.0.15` | Add iOS `accessibilityHidden` modifier wrapping SwiftUI's `accessibilityHidden(_:)`. Hides decorative views (hero icons, presentational imagery already described by adjacent text) from VoiceOver during element traversal. Registers `AccessibilityHiddenModifier` and exports `accessibilityHidden(hidden?)`, defaulting to `true`. | [expo/expo#46579](https://github.com/expo/expo/pull/46579) |
| [`hermes`](packages/hermes/) | n/a | Swap the hardcoded `hermes` source dir for `${{ github.event.repository.name }}` in the `test-linux-armv7` job's `cmake -S` and `test_runner.py` paths. The job uses `actions/checkout@v1` without `path:` (v4 breaks in the arm32 container, [actions/checkout#334](https://github.com/actions/checkout/issues/334)), so the checkout dir takes the repo name and the job fails on any fork not named `hermes`. Unchanged on `facebook/hermes`. | [facebook/hermes#2047](https://github.com/facebook/hermes/pull/2047) |
...
| :--- | :--- | :--- | :--- |
| [`@expo/ui`](packages/@expo/ui/) | `56.0.15` | Add iOS `accessibilityIdentifier` modifier wrapping SwiftUI's `accessibilityIdentifier(_:)`. Sets a stable, machine-readable id that UI-testing tools like XCUITest read to locate a view. Distinct from `accessibilityLabel`: not user-visible, for test targeting. Registers `AccessibilityIdentifierModifier` and exports `accessibilityIdentifier(id)`. | merged ([expo/expo#46556](https://github.com/expo/expo/pull/46556)) |
