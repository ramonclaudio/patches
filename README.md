...
| :--- | :--- | :--- | :--- |
| [`@expo/fingerprint`](packages/@expo/fingerprint/) | `0.19.3` | Hash each react-native core autolinking dependency by its `package.json` (`type: 'file'`) instead of its whole dir (`type: 'dir'`). On EAS workers, `pod install` and SDK 56 codegen write build artifacts into `node_modules/<dep>`, so the worker fingerprint drifts from a pristine local one and `eas build` fails `CONFIGURE_EXPO_UPDATES` with a runtime version mismatch under the `fingerprint` policy. Matches how `react-native` itself is already hashed via `packageJsonOnly`. A version bump still flips the hash and `rncoreAutolinkingConfig` still captures the resolved native config, so only build-artifact churn stops counting. Refs [#46323](https://github.com/expo/expo/issues/46323). | [expo/expo#46356](https://github.com/expo/expo/pull/46356) |
