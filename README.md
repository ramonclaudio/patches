...
| [`@hugeicons/react`](packages/@hugeicons/react/) | `1.1.6` | Bun, npm (patch-package), pnpm | `@hugeicons/core-free-icons` ships 5,100+ per-icon JS files but only a barrel `index.d.ts`. Subpath imports (`@hugeicons/core-free-icons/Heart`) work at runtime, but TypeScript can't find their types under `node16`, `nodenext`, or `bundler` resolution, so Vite dev pre-bundles the whole barrel. 6.2 MB instead of the 33 KB you actually use. Patch inlines a `declare module '@hugeicons/core-free-icons/*'` block at the top of `dist/types/index.d.ts` with the `IconSvgObject` union (`[string, { [key: string]: string \| number }][] \| readonly (readonly [string, { readonly [key: string]: string \| number }])[]`) the barrel exports. Single-file edit so `bun patchedDependencies` applies it cleanly without tripping [`oven-sh/bun#13330`](https://github.com/oven-sh/bun/issues/13330). | [hugeicons/react#5](https://github.com/hugeicons/react/pull/5) |
