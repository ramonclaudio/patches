...
| :--- | :--- | :--- | :--- |
| [`@expo/ui`](packages/@expo/ui/) | `56.0.16` | Add iOS `imageScale` modifier wrapping SwiftUI's `imageScale(_:)`. Scales SF Symbols relative to the surrounding text using the standard `small`, `medium`, and `large` sizes, and cascades from a container to descendant images like `controlSize`. Rounds out the symbol-sizing axis with `font({ textStyle })` ([#46007](https://github.com/expo/expo/pull/46007)) and `dynamicTypeSize` ([#46540](https://github.com/expo/expo/pull/46540)), and pairs with [#46714](https://github.com/expo/expo/pull/46714), which made those modifiers apply to `Image`. Registers `ImageScaleModifier` and exports `imageScale(scale)`. | merged ([expo/expo#46774](https://github.com/expo/expo/pull/46774)) |
| [`@expo/ui`](packages/@expo/ui/) | `56.0.16` | Fix `font`, `dynamicTypeSize`, and `resizable` modifiers on the `Image` component. `ImageView` pinned an internal `.font(.system(size: props.size ?? 24))` that overrode any `font` modifier (SwiftUI resolves `.font` nearest-to-leaf), and `resizable` was dropped because the wrapper runs the view-modifier pipeline, not the image pipeline. Routes `size` and `color` through `font` and `foregroundStyle` modifiers from JS (a supplied `font` always wins), drops the native pins, and runs `applyImageModifiers` on the concrete `Image` so `resizable` applies to symbols too. SF Symbols now scale with Dynamic Type via `font({ textStyle })`. | merged ([expo/expo#46714](https://github.com/expo/expo/pull/46714)) |
| [`@expo/ui`](packages/@expo/ui/) | `56.0.16` | Add iOS `accessibilityInputLabels` modifier wrapping SwiftUI's `accessibilityInputLabels(_:)`. Sets the alternative spoken phrases Voice Control listens for, so a control with a terse visible label like "End" answers to "Hang up" or "End call". The JS factory takes `string[]` and maps to `[Text]` natively. Registers `AccessibilityInputLabelsModifier` and exports `accessibilityInputLabels(labels)`. | merged ([expo/expo#46661](https://github.com/expo/expo/pull/46661)) |
