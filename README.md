...
| :--- | :--- | :--- | :--- | :--- |
| [`shadcn`](packages/shadcn/) | `4.7.0` | Bun, npm (patch-package), pnpm | Strip C0 control characters (0x00-0x1F) and DEL (0x7F) from `prompts` text input values; Cmd+Delete on macOS sends Ctrl+U (NAK) which `prompts` inserts as a literal byte, creating directories like `\x15my-app` | [shadcn-ui/ui#10364](https://github.com/shadcn-ui/ui/pull/10364) |
| [`bun`](packages/oven-sh/bun/) | `1.3.13` | Bun | Invalid YAML sequence in `update-root-certs` workflow `labels` field | [oven-sh/bun#27086](https://github.com/oven-sh/bun/pull/27086) |
| [`bun`](packages/oven-sh/bun/) | `1.3.13` | Bun | `includePrerelease` semantics wrong for peer dep semver validation | [oven-sh/bun#27085](https://github.com/oven-sh/bun/pull/27085) |
| [`better-auth`](packages/better-auth/) | `1.6.9` | Bun, npm (patch-package), pnpm | `/change-password` with `revokeOtherSessions: true` rotates the caller's cookie and session id because it deletes every session (current included) before minting a new one, contradicting the JSDoc ("revoke all sessions that are not the current one"); port the filter pattern from `/revoke-other-sessions` so only siblings get deleted and the caller's row survives; response `token` becomes `null` (sentinel already used by `sign-up`). Same family as [#9087](https://github.com/better-auth/better-auth/pull/9087); fixes the 500-1500 ms window where JWT-backed consumers (Convex) hold a token whose `sessionId` claim points at a deleted row | [better-auth/better-auth#9345](https://github.com/better-auth/better-auth/pull/9345) |
| [`@hugeicons/react`](packages/@hugeicons/react/) | `1.1.6` | Bun, npm (patch-package), pnpm | Ship `dist/core-free-icons.d.ts` declaring `@hugeicons/core-free-icons/*` with the `IconSvgObject` union (`[string, { [key: string]: string \| number }][] \| readonly (readonly [string, { readonly [key: string]: string \| number }])[]`) that `core-free-icons` itself uses for barrel exports, so subpath and barrel imports type identically; prepends a `/// <reference path="../core-free-icons.d.ts" />` directive to `dist/types/index.d.ts` so subpath type imports resolve under `node16`/`nodenext`/`bundler` resolution; fills the gap where `@hugeicons/core-free-icons` ships 5,100+ per-icon JS files but only a barrel `index.d.ts`, cutting dev pre-bundle from 6.2 MB to 33 KB for subpath-imported projects; emits the script .d.ts at `dist/` (one level above `dist/types/`) so `bun patchedDependencies` applies it cleanly without tripping [`oven-sh/bun#13330`](https://github.com/oven-sh/bun/issues/13330) | [hugeicons/react#5](https://github.com/hugeicons/react/pull/5) |
| [`expo`](packages/expo/) | n/a | Source (git apply) | `getPackageByName` (path lookup at `packages/<name>/package.json`) misses scoped packages whose dir name differs from the package name (`@expo/ui` lives at `packages/expo-ui/`, `@expo/app-integrity` at `packages/expo-app-integrity/`); when it misses, `Workspace.getInfoAsync` records empty `workspacePeerDependencies` so `updateWorkspaceProjects` never rewrites `workspace:*` peer entries to the canary version, leaving canary tarballs shipping `peerDependencies.expo: "workspace:*"` literal; `bun add @expo/ui@canary` and `npm install @expo/ui@canary` then fail with `Workspace dependency "expo" not found`; adds a name-based fallback that scans `cachedPackages` when the path lookup misses (cache is already populated by `getListOfPackagesAsync`); same root cause as [#44412](https://github.com/expo/expo/pull/44412) at a different call site | [expo/expo#45403](https://github.com/expo/expo/pull/45403) |
...
| :--- | :--- | :--- | :--- | :--- |
| [`@expo/ui`](packages/@expo/ui/) | `56.0.0-canary-20260212-4f61309` | Bun, npm (patch-package), pnpm | iOS: `scrollPosition` binding (iOS 17 `.scrollPosition(id:anchor:)`), `scrollPositionAnchor` prop, `onScrollPositionChangeSync` worklet callback, `id(string)` view modifier for scroll targets | `56.0.0-canary-20260505-d2856c3` ([expo/expo#44652](https://github.com/expo/expo/pull/44652)) |
| [`@convex-dev/better-auth`](packages/@convex-dev/better-auth/) | `0.11.5` | Bun, npm (patch-package), pnpm | Migrate to `better-auth` 1.6.9: peer `>=1.6.9 <1.7.0` (1.6.7/1.6.8 crash the Convex V8 isolate on every `/api/auth/*` via a bare `@opentelemetry/api` import from `@better-auth/core`'s adapter factory; [better-auth/better-auth#9340](https://github.com/better-auth/better-auth/pull/9340) routes it through a self-reference so `browser`/`edge` export conditions resolve to a noop); adapter validators accept `Where.mode` with case-folding so `api.adapter.findOne` stops throwing `ArgumentValidationError` on insensitive clauses; passes `asResponse: false` at 7 internal plugin endpoint sites so 1.6's flipped `shouldReturnResponse` default stops turning `{ token }` into `undefined`; delegates cross-domain `parseSetCookieHeader` to `better-auth/cookies` so cookies with RFC-1123 `Expires` dates stop shattering (local copy pre-dated [#8301](https://github.com/better-auth/better-auth/pull/8301)); adds `twoFactor.verified` schema; exposes `version` field on convex and cross-domain plugins; suppresses `@better-auth/oidc-provider` deprecation warning; picks up [GHSA-xr8f-h2gw-9xh6](https://github.com/better-auth/better-auth/security/advisories/GHSA-xr8f-h2gw-9xh6) in `@better-auth/oauth-provider` | `0.12.0` ([get-convex/better-auth#323](https://github.com/get-convex/better-auth/pull/323)) |
| [`better-auth`](packages/better-auth/) | n/a | Source (git apply) | Add a `./instrumentation` subpath with `browser`/`edge` export conditions pointing to `pure.index.mjs` (noop) so runtimes that reject bare specifiers at resolve time (Convex V8 isolate, where `deno_core::resolve_import` throws synchronously instead of rejecting the `import("@opentelemetry/api")` promise) can resolve instrumentation without loading `@opentelemetry/api`; pairs with [#9340](https://github.com/better-auth/better-auth/pull/9340) which swaps the adapter factory from a relative `./instrumentation` import to the package self-reference so the conditions actually kick in (chain completed in `1.6.9`) | `1.6.7` ([better-auth/better-auth#9281](https://github.com/better-auth/better-auth/pull/9281)) |
| [`shadcn/ui`](packages/shadcn-ui/) | n/a | Source (git apply) | `start-app` and `start-monorepo` templates print `A notFoundError was encountered on the route with ID "__root__"` on first load because phantom requests (`/favicon.ico`, Chrome DevTools `/.well-known/appspecific/com.chrome.devtools.json`) hit routes that don't exist and no `notFoundComponent` is configured; adds a 404 handler to both template root routes matching `react-router-app`'s existing `ErrorBoundary` 404 (same `container mx-auto p-4 pt-16` wrapper, same copy) | merged ([shadcn-ui/ui#10369](https://github.com/shadcn-ui/ui/pull/10369)) |
| [`shadcn/ui`](packages/shadcn-ui/) | n/a | Source (git apply) | Add TanStack Start as a fifth dark mode guide alongside Next.js, Vite, Astro, and Remix; new MDX guide with `ScriptOnce` + Context pattern (SSR-safe `useState`, `suppressHydrationWarning`, `colorScheme`, OS preference listener); index card with TanStack logo; `meta.json` entry | merged ([shadcn-ui/ui#10396](https://github.com/shadcn-ui/ui/pull/10396)) |
...
| :--- | :--- | :--- | :--- | :--- |
| [`expo-router`](packages/expo-router/) | `56.0.0-canary-20260423-c31bd8e` | Bun, npm (patch-package), pnpm | Infinite render loop in all 5 Stack composition components (`Toolbar`, `Header`, `Screen.Title`, `SearchBar`, `Screen.BackButton`) when props include unstable references; adds `useStableCompositionOption` helper with structural fingerprinting; fix for issue [expo/expo#44561](https://github.com/expo/expo/issues/44561) | closed by me ([expo/expo#44563](https://github.com/expo/expo/pull/44563); upstream likely fixed the symptom between filing and now, will refile if it returns) |
| [`@convex-dev/better-auth`](packages/@convex-dev/better-auth/) | `0.11.4` | Bun, npm (patch-package), pnpm | Clear `cachedToken` and `pendingTokenRef` when `useSession()` reports a different `sessionId` so a session rotation (`changePassword({ revokeOtherSessions: true })`, cross-domain handoff, custom plugin endpoints) doesn't leave the Convex client holding a JWT bound to the dead session; `fetchAccessToken` already rebuilds on `[sessionId]` but the cached token state is only cleared in the logout-only effect, so when `sessionId` flips with `session` still truthy the rebuilt closure reads the same stale token and Convex's `setConfig` short-circuits on the stale value | closed by me ([get-convex/better-auth#329](https://github.com/get-convex/better-auth/pull/329); superseded by [better-auth/better-auth#9345](https://github.com/better-auth/better-auth/pull/9345) which fixes the upstream root cause instead of patching the symptom client-side) |
...
| [`@shopify/mini-oxygen`](packages/@shopify/mini-oxygen/) | `4.0.0` | Bun, npm (patch-package), pnpm | Vite 7 `ReferenceError: __vite_ssr_exportName__ is not defined`; missing 6th SSR key, `getBuiltins()` support, `fetchModule` importer fix, removes deprecated `root`, bumps `vite` peer dep to `^7.0.0` | next release ([Shopify/hydrogen#3617](https://github.com/Shopify/hydrogen/pull/3617); my PR [#3493](https://github.com/Shopify/hydrogen/pull/3493) closed, upstream rebuilt with Vite Environment API for 6/7/8 backward compat) |
...
| [oven-sh/bun#29444](https://github.com/oven-sh/bun/issues/29444) | `bun` | open | `bun install` warns on valid prereleases in peer deps because `satisfies` ignores `includePrerelease` semantics; my PR [#27085](https://github.com/oven-sh/bun/pull/27085) ports the `satisfiesIncludePrerelease` semver path to peer dep validation |
| [expo/expo#44561](https://github.com/expo/expo/issues/44561) | `expo-router` | closed (no repro) | `Stack.Toolbar` with `placement='left'` or `'right'` causes infinite render loop on any parent re-render; auto-closed by `expo-bot` for a missing minimal repro template; my workaround [#44563](https://github.com/expo/expo/pull/44563) was later closed without merge after upstream likely fixed the symptom |
| [expo/expo#44564](https://github.com/expo/expo/issues/44564) | `expo-router` | closed by me | `useCompositionOption` has an implicit "stable identity" contract that callers violate with inline JSX children; closed by me as redundant after documenting the architectural concern in the [#44563](https://github.com/expo/expo/pull/44563) PR body, which itself was later closed without merge |
| [Shopify/hydrogen#3263](https://github.com/Shopify/hydrogen/issues/3263) | `@shopify/mini-oxygen` | fixed | Vite 7 support in `@shopify/mini-oxygen`; closed by me after [@frandiox](https://github.com/frandiox) shipped Vite Environment API in [#3617](https://github.com/Shopify/hydrogen/pull/3617) (supersedes my rebase in [#3493](https://github.com/Shopify/hydrogen/pull/3493)) |
