...
Once the fix ships in a release, drop the patch and bump the dep like you normally would.

This is me sending patches.

## Open

PRs still open upstream, not merged yet so the fix could still change. Every row has a patch in `packages/` you can drop into your project.

| Package | Version | Fix | PR |
| :--- | :--- | :--- | :--- |
| [`hermes`](packages/hermes/) | n/a | Cherry-pick `1e94fbe0eb` onto `250829098.0.0-stable`. A `finally` block emits its body twice; on the second emission legacy class compilation built fresh internal Variables (class and prototype vars, the instance-element init function, computed field keys, private brands) while cached functions reached via `findCompiledEntity` still referenced the originals, miscompiling classes declared in `finally`. Caches them in a `LegacyClassVars` struct keyed by the class AST node and emits private-brand stores unconditionally. Source fix for the class-in-finally bug from [facebook/hermes#1761](https://github.com/facebook/hermes/issues/1761). | [facebook/hermes#2046](https://github.com/facebook/hermes/pull/2046) |
...
| [`react-native`](packages/react-native/) | `0.85.3` | Set `:always_out_of_date => "1"` on `hermes-engine.podspec`'s `[Hermes] Replace Hermes for the right configuration, if needed` script_phase. The phase had no declared outputs (overwrites the prebuilt Hermes binary in place per `$CONFIGURATION`), so Xcode 14+ was warning about it on every clean build of every project on the default prebuilt-release-tarball Hermes path. Matches the family pattern from `React-Core-prebuilt.podspec` ([#52133](https://github.com/react/react-native/pull/52133)) and `ReactNativeDependencies.podspec` ([#49812](https://github.com/react/react-native/pull/49812)). | [react/react-native#56912](https://github.com/react/react-native/pull/56912) |
| [`bun`](packages/oven-sh/bun/) | `1.3.14` | Fix `bun add X@version` being silently ignored when `X` is a `peerDependency` on the same name: `package.json` bumped while `bun.lock` and `node_modules` stayed pinned to whatever the peer resolved to first. Drops the order-dependent peer-dep early-match in `get_or_put_resolved_package` (which also made `bun.lock` vary run to run, since `package_index` fills in thread-pool order) and routes peers through `find_best_version`, with dedup in `Tree::hoist_dependency` where placement is deterministic. Rust port of Dylan's [#29804](https://github.com/oven-sh/bun/pull/29804). | [oven-sh/bun#30855](https://github.com/oven-sh/bun/pull/30855) |
...
Merged upstream. `Fixed in` is the version to bump to, then delete the patch. If it says `unreleased`, the fix merged but isn't in a published version yet, so keep the patch until it lands in one.

| Package | Was | Fix | Fixed in |
| :--- | :--- | :--- | :--- |
| [`expo-router`](packages/expo-router/) | `57.0.3` | Add `testID` and `accessibilityLabel` props to `NativeTabs.Trigger`. `testID` maps to the tab bar item's `accessibilityIdentifier` on iOS (so XCUITest and Maestro can target tabs) and its view tag on Android; `accessibilityLabel` sets the item's screen-reader label (Android `contentDescription`, iOS 26+). The native `NativeTabsView` already accepts `tabBarItemTestID` and `tabBarItemAccessibilityLabel`; this wires the two JS props through `convertTabPropsToOptions`. Patches the compiled `build/native-tabs/` runtime and types. | unreleased ([expo/expo#47472](https://github.com/expo/expo/pull/47472)) |
| [`@expo/ui`](packages/@expo/ui/) | `57.0.2` | Add iOS `strokeBorder` modifier wrapping SwiftUI's `InsettableShape.strokeBorder(_:style:antialiased:)`. Strokes an inset border that follows the view's shape with a full `StrokeStyle` (line width, cap, join, miter limit, dash array, dash phase), so a border can be dashed or dotted and hug rounded corners instead of only a solid rectangle. Takes an optional `color` (omit for the foreground style), `style`, `antialiased`, a `shape` (`rectangle`, `circle`, `capsule`, `ellipse`, `roundedRectangle`, `containerRelativeShape`), and `cornerRadius`. Registers `StrokeBorderModifier` and exports `strokeBorder(params)`. | unreleased ([expo/expo#47426](https://github.com/expo/expo/pull/47426)) |
| [`@expo/ui`](packages/@expo/ui/) | `56.0.19` | Add iOS `accessibilityAddTraits` and `accessibilityRemoveTraits` modifiers wrapping SwiftUI's `accessibilityAddTraits(_:)` and `accessibilityRemoveTraits(_:)`. Tag a view with semantic traits for VoiceOver (`isButton`, `isHeader`, `isImage`, `isSelected`, `isModal`, `isSummaryElement`, and more) or strip inherited ones. Both take an `AccessibilityTrait[]`. `isToggle` and `isTabBar` need iOS 17. Registers `AccessibilityAddTraitsModifier` and `AccessibilityRemoveTraitsModifier`, and exports the two factories. | `56.0.20` ([expo/expo#47387](https://github.com/expo/expo/pull/47387)) |
| [`@expo/ui`](packages/@expo/ui/) | `56.0.18` | Add iOS `redacted`, `unredacted`, `privacySensitive`, and `invalidatableContent` modifiers wrapping SwiftUI's redaction family. `redacted('placeholder')` swaps a view's subtree for skeleton placeholders. `redacted('privacy')` and `redacted('invalidated')` redact only descendants marked `privacySensitive()` or `invalidatableContent()`, and `unredacted()` exempts a subtree from a redacted ancestor. Reasons are additive through an array. `invalidated` needs iOS 17. Registers `RedactedModifier`, `UnredactedModifier`, `PrivacySensitiveModifier`, and `InvalidatableContentModifier`, and exports the four factories. | `57.0.1` ([expo/expo#47269](https://github.com/expo/expo/pull/47269)) |
| [`@expo/ui`](packages/@expo/ui/) | `56.0.18` | Add iOS `accessibilityElement` modifier wrapping SwiftUI's `accessibilityElement(children:)`. Collapses a view's subtree into a single accessibility element so VoiceOver reads grouped children (a star icon next to a "4.8" label) as one "4.8 out of 5 stars" element instead of stopping on each. Takes `ignore` (default), `combine`, or `contain` via `AccessibilityChildBehavior`. Complements `accessibilityHidden` ([#46579](https://github.com/expo/expo/pull/46579)), which hides a single leaf. Registers `AccessibilityElementModifier` and exports `accessibilityElement(children?)`. | `57.0.0` ([expo/expo#47156](https://github.com/expo/expo/pull/47156)) |
| [`@expo/ui`](packages/@expo/ui/) | `56.0.16` | Add iOS `imageScale` modifier wrapping SwiftUI's `imageScale(_:)`. Scales SF Symbols relative to the surrounding text using the standard `small`, `medium`, and `large` sizes, and cascades from a container to descendant images like `controlSize`. Rounds out the symbol-sizing axis with `font({ textStyle })` ([#46007](https://github.com/expo/expo/pull/46007)) and `dynamicTypeSize` ([#46540](https://github.com/expo/expo/pull/46540)), and pairs with [#46714](https://github.com/expo/expo/pull/46714), which made those modifiers apply to `Image`. Registers `ImageScaleModifier` and exports `imageScale(scale)`. | `56.0.17` ([expo/expo#46774](https://github.com/expo/expo/pull/46774)) |
| [`@expo/ui`](packages/@expo/ui/) | `56.0.16` | Fix `font`, `dynamicTypeSize`, and `resizable` modifiers on the `Image` component. `ImageView` pinned an internal `.font(.system(size: props.size ?? 24))` that overrode any `font` modifier (SwiftUI resolves `.font` nearest-to-leaf), and `resizable` was dropped because the wrapper runs the view-modifier pipeline, not the image pipeline. Routes `size` and `color` through `font` and `foregroundStyle` modifiers from JS (a supplied `font` always wins), drops the native pins, and runs `applyImageModifiers` on the concrete `Image` so `resizable` applies to symbols too. SF Symbols now scale with Dynamic Type via `font({ textStyle })`. | `56.0.17` ([expo/expo#46714](https://github.com/expo/expo/pull/46714)) |
| [`@expo/ui`](packages/@expo/ui/) | `56.0.16` | Add iOS `accessibilityInputLabels` modifier wrapping SwiftUI's `accessibilityInputLabels(_:)`. Sets the alternative spoken phrases Voice Control listens for, so a control with a terse visible label like "End" answers to "Hang up" or "End call". The JS factory takes `string[]` and maps to `[Text]` natively. Registers `AccessibilityInputLabelsModifier` and exports `accessibilityInputLabels(labels)`. | `56.0.17` ([expo/expo#46661](https://github.com/expo/expo/pull/46661)) |
| [`hermes`](packages/hermes/) | n/a | Swap the hardcoded `hermes` source dir for `${{ github.event.repository.name }}` in the `test-linux-armv7` job's `cmake -S` and `test_runner.py` paths. The job uses `actions/checkout@v1` without `path:` (v4 breaks in the arm32 container, [actions/checkout#334](https://github.com/actions/checkout/issues/334)), so the checkout dir takes the repo name and the job fails on any fork not named `hermes`. Unchanged on `facebook/hermes`. | merged ([facebook/hermes#2047](https://github.com/facebook/hermes/pull/2047)) |
| [`@expo/ui`](packages/@expo/ui/) | `56.0.15` | Add iOS `accessibilityHidden` modifier wrapping SwiftUI's `accessibilityHidden(_:)`. Hides decorative views (hero icons, presentational imagery already described by adjacent text) from VoiceOver during element traversal. Registers `AccessibilityHiddenModifier` and exports `accessibilityHidden(hidden?)`, defaulting to `true`. | `56.0.16` ([expo/expo#46579](https://github.com/expo/expo/pull/46579)) |
| [`@expo/ui`](packages/@expo/ui/) | `56.0.15` | Add iOS `accessibilityIdentifier` modifier wrapping SwiftUI's `accessibilityIdentifier(_:)`. Sets a stable, machine-readable id that UI-testing tools like XCUITest read to locate a view. Distinct from `accessibilityLabel`: not user-visible, for test targeting. Registers `AccessibilityIdentifierModifier` and exports `accessibilityIdentifier(id)`. | `56.0.16` ([expo/expo#46556](https://github.com/expo/expo/pull/46556)) |
| [`@expo/ui`](packages/@expo/ui/) | `56.0.15` | Add iOS `dynamicTypeSize` modifier to set or clamp Dynamic Type within a view. Pass a single size to pin it, or `{ min, max }` to bound the range (either end optional). Caps how far text grows at the largest accessibility sizes for layout safety while still honoring Dynamic Type. Bounds the `textStyle` scaling from [#46007](https://github.com/expo/expo/pull/46007). Set it on `<Host>` to cascade to descendants. Accepts the 12 `DynamicTypeSize` cases, `xSmall` through `accessibility5`. | `56.0.16` ([expo/expo#46540](https://github.com/expo/expo/pull/46540)) |
| [`@expo/ui`](packages/@expo/ui/) | `56.0.15` | Resolve the `font` modifier on the Text-concatenation path. A `<Text>` nested in another `<Text>` built a fixed-size `Font.custom` that dropped `relativeTo` (Dynamic Type) and `weight`, so `font({ textStyle, weight })` scaled standalone but lost both once concatenated. Makes `FontModifier.resolveFont()` non-private and calls it from both paths, so concatenated runs resolve the identical `Font`. Completes [#46007](https://github.com/expo/expo/pull/46007). | `56.0.16` ([expo/expo#46509](https://github.com/expo/expo/pull/46509)) |
