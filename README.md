...
| [`@hugeicons/react`](packages/@hugeicons/react/) | `1.1.6` | Bun, npm (patch-package), pnpm | Ship `dist/types/core-free-icons.d.ts` declaring `@hugeicons/core-free-icons/*` with the inlined `IconSvgObject` union (`[string, { [key: string]: string \| number }][] \| readonly (readonly [string, { readonly [key: string]: string \| number }])[]`) that `core-free-icons` itself uses for barrel exports, so subpath and barrel imports type identically; prepends a `/// <reference path="./core-free-icons.d.ts" />` directive to `dist/types/index.d.ts` so subpath type imports resolve under `node16`/`nodenext`/`bundler` resolution; fills the gap where `@hugeicons/core-free-icons` ships 5,100+ per-icon JS files but only a barrel `index.d.ts`, cutting dev pre-bundle from 6.2 MB to 33 KB for subpath-imported projects | [hugeicons/react#5](https://github.com/hugeicons/react/pull/5) |
