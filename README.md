...
| [`@hugeicons/react`](packages/@hugeicons/react/) | `1.1.6` | Bun, npm (patch-package), pnpm | Inline `declare module '@hugeicons/core-free-icons/*'` with the `IconSvgObject` union (`[string, { [key: string]: string \| number }][] \| readonly (readonly [string, { readonly [key: string]: string \| number }])[]`) directly into `dist/types/index.d.ts` so subpath type imports resolve under `node16`/`nodenext`/`bundler` resolution and barrel and subpath imports type identically; fills the gap where `@hugeicons/core-free-icons` ships 5,100+ per-icon JS files but only a barrel `index.d.ts`, cutting dev pre-bundle from 6.2 MB to 33 KB for subpath-imported projects; single-file edit so `bun patchedDependencies` applies cleanly without tripping [`oven-sh/bun#13330`](https://github.com/oven-sh/bun/issues/13330) | [hugeicons/react#5](https://github.com/hugeicons/react/pull/5) |
