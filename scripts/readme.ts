// Accepts a single version, a bare marker, or a comma-separated list when the
// fix shipped on several release lines (backport + mainline).
export function fmtVersion(version: string): string {
  return version
    .split(", ")
    .map((v) => (v === "merged" || v === "n/a" || v === "unreleased" ? v : `\`${v}\``))
    .join(", ");
}
...
export function moveOpenRowToMerged(
  ...
  // Already in Merged: drop a lingering stale Open row, never append a duplicate.
  if (mergedMatch[1].split("\n").some((l) => rowMatchesPr(l, prNumber, owner, repo))) {
    if (rowIdx === -1) return readme;
    const cleaned = lines.filter((_, i) => i !== rowIdx).join("\n");
    return readme.replace(openSection, () => cleaned);
  }
  ...
  const newMergedSection = [
    ...mergedLines.slice(0, sepIdx + 1),
    newRow,
    ...mergedLines.slice(sepIdx + 1),
  ].join("\n");
...
export function updateMergedRowVersion(
  ...
  // Growing the version list (a later release line ships the same fix) is
  // additive and safe; replacing a recorded version still needs --force.
  const curSet = current.split(",").map((s) => s.trim());
  const newSet = version.split(",").map((s) => s.trim());
  const additive = current === "unreleased" || curSet.every((v) => newSet.includes(v));
  if (!additive && !force) return readme;
...
export function parseOpenRows(readme: string): { pkg: string; version: string; pr: Pr | null }[] {
